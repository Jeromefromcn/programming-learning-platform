...
class StudentDraftServiceTest {
    ...
    @Test
    void saveDraft_existingDraft_overwritesInPlace() {
        when(exerciseRepo.findByIdAndDeletedFalse(2L)).thenReturn(Optional.of(publishedExercise()));

        ExerciseDraft existingDraft = new ExerciseDraft();
        existingDraft.setId(55L);
        existingDraft.setUserId(1L);
        existingDraft.setExerciseId(2L);
        existingDraft.setExerciseType("PYTHON");
        existingDraft.setAnswerData("old code");

        when(draftRepo.findByUserIdAndExerciseId(1L, 2L)).thenReturn(Optional.of(existingDraft));
        when(draftRepo.save(any())).thenAnswer(inv -> inv.getArgument(0));

        DraftDto updated = service.saveDraft(1L, 2L, new SaveDraftRequest("new code", null));
        assertEquals("new code", updated.answerData());

        verify(draftRepo).save(argThat(d ->
            d.getId() != null && d.getId().equals(55L) && "new code".equals(d.getAnswerData())));
    }
